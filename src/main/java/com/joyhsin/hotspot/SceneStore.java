...
public class SceneStore {
  ...
  public SceneStore(ObjectMapper mapper,AppProperties props) throws IOException { this.mapper=mapper; file=props.dataDirectory().toAbsolutePath().normalize().resolve("scenes.json"); Files.createDirectories(file.getParent()); if(Files.notExists(file)) write(List.of()); }

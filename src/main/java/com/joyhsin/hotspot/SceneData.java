...
public record SceneData(String id,String name,String imageUrl,List<Hotspot> draftHotspots,List<Hotspot> publishedHotspots,Instant updatedAt,Instant publishedAt) {
  ...
  public record Hotspot(@NotBlank String id,@DecimalMin("0") @DecimalMax("1") double xRatio,@DecimalMin("0") @DecimalMax("1") double yRatio,@NotBlank @Size(max=80) String label,@Pattern(regexp="^#[0-9a-fA-F]{6}$") String markerColor,@Valid @NotNull Product product) {}
